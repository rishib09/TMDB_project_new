...
|---|---|---|
| Carry state between turns | `graph.compile(checkpointer=InMemorySaver())` + `config={"configurable": {"thread_id": ...}}`; `graph.get_state`, `get_state_history` (`langgraph.checkpoint.memory`) | the per-turn scratch reset node and the history trim node |
...
| Trim message history | `RemoveMessage`, `REMOVE_ALL_MESSAGES` (`langgraph.graph.message`) | window size (config) |
...
| Capability | Installed API | Stays hand-written |
|---|---|---|
| Build a chat client | `init_chat_model(model, model_provider="openai", base_url=, api_key=, max_tokens=, timeout=, max_retries=, reasoning_effort=)` (`langchain.chat_models`); one factory, used by router, synthesizer, judge | model ids in `ExperimentConfig` (ADR 0004) |
...
| LLM response cache | `set_llm_cache` exists — **forbidden** without latency evidence (ADR 0009); global and cross-user | — |

## Retrieval, indexing, storage — raw libraries by decision (D19)

| Capability | Installed API | Note |
|---|---|---|
